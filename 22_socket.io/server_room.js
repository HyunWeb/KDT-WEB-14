...
io.on("connection", (socket) => {
  ...
  socket.on("create", ({ roomName, nickname }, cb) => {
    ...
    socket.to(roomName).emit("notice", `${nickname} 님이 입장하셨습니다.`);
  ...
  socket.on("setNick", (nickname) => {
    ...
    // 이미 존재하는 닉네임이라면 error 이벤트 + '이미 존재하는 닉네임 입니다.' => 클라이언트; error 이벤트 받고, alert 띄워줌
    // 새 닉네임; notice 이벤트

    //[퀴즈] 존재하는 닉네임인지 검사, 후 처리
    if (Object.values(nickObjs).includes(nickname)) {
  ...
  socket.on("send", ({ dm, nick, msg, room }) => {
    ...
    if (dm === "all") {
      ...
      socket.emit("showMessage", data); // 자신한테 보내는 메세지
    }
  });
